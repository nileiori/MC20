...
static s32 ATRsp_QNITZ_Handler(char* line, u32 len, void* param)
{
    char* pHead = Ql_RIL_FindString(line, len, "+QNITZ:");
    if (pHead)
    {
        Ql_sscanf(pHead,"%*[^: \"]: \"%[^\"\r\n]", (char*)param);
        return  RIL_ATRSP_CONTINUE; // wait for OK
...
/******************************************************************************
* Function:     RIL_Sync_Network_Time  同步网络时间
*  
* Description:
...
* Parameters:    
*               networkTime:
*                   [out] networkTime number, a string of 20-byte.
* Return:  
 *                RIL_AT_SUCCESS, this function succeeds.
 *                Or, please see the definition of Enum_ATSndError.
******************************************************************************/
s32 RIL_Sync_Network_Time(char* networkTime)
{
    char strAT[] = "AT+CTZU=3\0";
    if (NULL == networkTime)
    {
        return RIL_AT_INVALID_PARAM;
    }
    return Ql_RIL_SendATCmd(strAT,Ql_strlen(strAT), ATRsp_QNITZ_Handler,(void*)networkTime, 0);
}
